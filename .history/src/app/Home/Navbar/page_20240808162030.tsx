...
function Navbar({ className }: { className?: string }) {
  ...
  const renderLinks = (links: LinkType[]) => (
    links.map(link => (
      <HoveredLink key={link.name} href={link.href} className="text-lg leading-relaxed tracking-wide mb-2 last:mb-0">
        {link.name}
      </HoveredLink>
    ))
  );
    ...
    <div className={cn("fixed top-0 inset-x-0 bg-black text-white z-50 mt-2", className)}>
      <div className="max-w-7xl mx-auto px-4 flex justify-between items-center h-20">
        <div className="flex justify-start items-center flex-1">
          <div className="relative w-20 h-20 mr-4">  // Increased logo size
            <Image
              src="/QuadTree.svg"
              alt="QuadTree Logo"
              layout="fill"
