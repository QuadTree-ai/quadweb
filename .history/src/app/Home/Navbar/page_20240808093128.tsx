...
function Navbar({ className = "" }: { className?: string }) {
  ...
  const handleSetActive = (name: string) => {
    // Toggle active state: if it's already active, set to null; otherwise, set to the section name
    ...
    <div className={cn("fixed top-10 inset-x-0 max-w-2xl mx-auto z-50", className)}>
      <Menu setActive={setActive}>
        {navData.map((section: NavSection, index: number) => (
          <MenuItem
            key={index}
            setActive={() => handleSetActive(section.name)}
            active={active === section.name ? section.name : null} // Correctly pass string or null
            item={section.name}
          >
