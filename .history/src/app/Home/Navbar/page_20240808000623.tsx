...
function Navbar({ className }: { className?: string }) {
  ...
  const renderMenuContent = (column: { title?: string; links: any; image: any; }) => (
    <div className="flex flex-col space-y-4 text-sm">
      {renderLinks(column.links)}
      {column.image && (
        <img src={column.image.src} alt={column.image.alt} className="w-32 h-20 mt-2" />
      )}
    </div>
  );
    ...
    <div className={cn("fixed top-10 inset-x-0 max-w-2xl mx-auto z-50", className)}>
      <Menu setActive={setActive}>
        {navData.map((section, idx) => (
          <MenuItem key={idx} setActive={setActive} active={active} item={section.name}>
