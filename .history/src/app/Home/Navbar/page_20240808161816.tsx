...
function Navbar({ className }: { className?: string }) {
  ...
  const renderMenuItems = () => (
    navData.map((section: SectionType) => (
      <MenuItem key={section.name} setActive={setActive} active={active} item={section.name}>
        <div className={`flex justify-start items-start space-x-12 p-6`}>  // Increased spacing for better visual separation
  ...
  return (
    <div className={cn("fixed top-0 inset-x-0 bg-black text-white z-50", className)}>
      <div className="max-w-7xl mx-auto px-8 flex justify-between items-center h-24">  // Increased height and horizontal padding
        <div className="flex justify-start items-center">
          <div className="relative w-20 h-20 mr-8">  // Increased logo size for better visibility
            <Image
              src="/QuadTree.svg"
              alt="QuadTree Logo"
              layout="fill"
