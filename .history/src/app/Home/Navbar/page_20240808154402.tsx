...
function Navbar({ className }: { className?: string }) {
  ...
  const renderLinks = (links: LinkType[]) => (
    links.map(link => (
      <HoveredLink key={link.name} href={link.href} className="text-lg leading-loose tracking-wide text-neutral-600">
        {link.name}
      </HoveredLink>
    ))
  );
  ...
  const renderMenuItems = () => (
    navData.map((section: SectionType) => (
      <MenuItem key={section.name} setActive={setActive} active={active} item={section.name}>
        <div className={`flex justify-start items-start space-x-8 p-4`}>
