...
function Navbar({ className = "" }: { className?: string }) {
  ...
  // Function to render products if available in the data
  const renderProducts = (products: any[]) => {
    return products.map((product: { title: React.Key | null | undefined; href: string; src: string; description: string; }) => (
      <ProductItem
        key={product.title}
        title={product.title}
        href={product.href}
          ...
          <MenuItem key={section.name} setActive={setActive} active={active} item={section.name}>
            {section.columns.map((column, idx) => (
              <div key={idx} className="space-y-4 text-sm">
                ...
                {/* Optionally render products if they exist in the column data */}
                {column.products && renderProducts(column.products)}
              </div>
            ))}
          </MenuItem>
        ))}
