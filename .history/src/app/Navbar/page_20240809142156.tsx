...
function Navbar({ className }: { className?: string }) {
  ...
  const renderColumns = (columns: ColumnType[]) =>
    columns.map((column, index) => (
      <div
        key={column.title}
        className={`flex flex-col space-y-3 ${index < columns.length - 1 ? "md:mr-8" : ""}`}
      >
  ...
  return (
    ...
    >
      <div className="max-w-full mx-auto px-4 md:px-6 flex flex-col md:flex-row justify-between items-center h-20 mt-6 mb-2">
        {/* Logo and Hamburger */}
